...
object TimeTest {
  def main(args: Array[String]): Unit = {
    ...
    val resultStream = inputStream
      ...
      .assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor[SensorReading](Time.seconds(20)){
      ...
      .allowedLateness(Time.seconds(1))    //允许 1 minute 钟的延时, 等到 1 minutes 后，才关闭窗口，在延长的窗口内每来一条一个数据，更新输出一次结果
      .sideOutputLateData(new OutputTag[SensorReading]("late"))  //如果1 s 后才来的数据，可以用侧输出流
...
//
//    val lateDataStream = resultStream.keyBy("id")
...
//        .sideOutputLateData(new OutputTag[SensorReading]("late"))  //如果1 minute 后才来的数据，可以用侧输出流
